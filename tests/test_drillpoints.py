...
def test_item_stats(point_one_item, real_item):
    ...
    item_stats = drillpoints.ItemStats(point_one_item, real_item)
    assert item_stats.item.id == "S2B_53HPV_20220728_0_L2A"
    assert item_stats.stats == {
        drillpoints.STATS_RAW: [],
        drillpoints.STATS_ARRAYINFO: []}
...
def test_read_data(caplog, point_one_item, real_item):
    ...
    raw_stats = item_stats.get_stats(drillpoints.STATS_RAW)
    ...
    ip.calc_stats(std_stats=[drillpoints.STATS_COUNT])
    item_stats = point_one_item.get_item_stats(real_item.id)
    raw_stats = item_stats.get_stats(drillpoints.STATS_RAW)
    assert len(raw_stats) == 0
    counts = item_stats.get_stats(drillpoints.STATS_COUNT)
    assert len(counts) == 0
...
def test_calc_stats_image(point_one_item, real_image_path):
    ...
    ip.add_point(point_one_item)
    ip.read_data()
    std_stats = [drillpoints.STATS_COUNT]
...
def test_calc_stats(point_one_item, real_item):
    """Test the calc_stats methods of both drillpoints and ItemStats."""
    # Standard functions.
    std_stats = [
        drillpoints.STATS_RAW, drillpoints.STATS_MEAN, drillpoints.STATS_STDEV,
    ...
    item_stats = point_one_item.get_item_stats(real_item.id)
    # Standard stats
    assert drillpoints.STATS_RAW in item_stats.stats
    assert drillpoints.STATS_MEAN in item_stats.stats
    assert drillpoints.STATS_STDEV in item_stats.stats
    assert drillpoints.STATS_COUNT in item_stats.stats
    assert drillpoints.STATS_COUNTNULL in item_stats.stats
    mean_vals = item_stats.get_stats(drillpoints.STATS_MEAN)
    counts = item_stats.get_stats(drillpoints.STATS_COUNT)
    null_counts = item_stats.get_stats(drillpoints.STATS_COUNTNULL)
    ...
    stdev = point_one_item.get_stat(real_item.id, drillpoints.STATS_STDEV)
...
def test_check_std_arrays(fake_item):
    ...
    with pytest.raises(drillpoints.MultibandAssetError) as excinfo:
        drillpoints.check_std_arrays(fake_item, [a1, mba, a3])
...
def test_std_stat_mean():
    """Test drillpoints.std_stat_mean."""
    # All arrays are 3D.
    ...
    a3 = numpy.ma.arange(4,16).reshape((1,4,3))
    mean_vals = drillpoints.std_stat_mean([a1, a2, a3])
...
def test_std_stat_count():
    ...
    counts = drillpoints.std_stat_count([m_a1, m_a2, m_a3])
    assert list(counts) == [9, 16, 0]


def test_std_stat_countnull():
    """Test drillpoints.std_stat_countnull."""
    a1 = numpy.arange(10).reshape((1,2,5))
    m_a1 = numpy.ma.masked_array(a1, mask=a1<3)
    m_a2 = numpy.ma.arange(4,20).reshape((1,4,4))
    m_a3 = numpy.ma.masked_array([], mask=True)
    counts = drillpoints.std_stat_countnull([m_a1, m_a2, m_a3])
...
def test_handle_nulls(point_partial_nulls, point_all_nulls, real_item):
    ...
    std_stats = [drillpoints.STATS_MEAN]
...
def test_handle_outofrange(
    ...
    raw_b02 = item_stats.get_stats(drillpoints.STATS_RAW)[0]
    ...
    mean_b02 = item_stats.get_stats(drillpoints.STATS_MEAN)[0]
...
def test_reset(point_one_item, real_item):
    ...
    assert list(i_stats.stats.keys()) == [
        drillpoints.STATS_RAW, drillpoints.STATS_ARRAYINFO, drillpoints.STATS_MEAN]
    assert len(point_one_item.get_stat(real_item.id, drillpoints.STATS_MEAN)) == 2
    ...
    std_stats.append(drillpoints.STATS_COUNT)
    ip.set_asset_ids(['B8A'])
    ip.read_data()
    ip.calc_stats(std_stats=std_stats)
    i_stats = list(point_one_item.get_stats().values())[0]
    assert list(i_stats.stats.keys()) == [
        drillpoints.STATS_RAW, drillpoints.STATS_ARRAYINFO,
        drillpoints.STATS_MEAN, drillpoints.STATS_COUNT]
    assert len(point_one_item.get_stat(real_item.id, drillpoints.STATS_MEAN)) == 3
    ...
    i_stats = list(point_one_item.get_stats().values())[0]
    assert list(i_stats.stats.keys()) == [
        drillpoints.STATS_RAW, drillpoints.STATS_ARRAYINFO, drillpoints.STATS_COUNT]
    assert len(point_one_item.get_stat(real_item.id, drillpoints.STATS_COUNT)) == 1
