...
class Point:
    ...
    def add_data(self, item, arr_info):
        ...
        arr_info is the ArrayInfo object generated by reading pixel data from
        on of the item's assets. See asset_reader.AssetReader.read_roi().
...
class ItemPoints(PointCollection):
    ...
    def read_data(self, asset_ids, ignore_val=None):
        ...
        Once read, the ItemStats objects for each point each Point in this collection 
        Once read, every ItemStats object of every Point will have a RAW_STATS
        attribute, which is a list of 3D arrays. There's one element in the list
        for every asset corresponding to asset_ids.
...
class ItemStats:
    ...
    def add_data(self, arr_info):
        """
        Add the pointstats.ArrayInfo.

        arr_info.data is the numpy masked array of data, which contains the
        pixels for one of the assets of the item.

        arr_info.data is appended to the list in self.stats[STATS_RAW].
        arr_info is appended to the list in self.stats[STATS_ARRAYINFO].
    ...
    def calc_stats(self, std_stats, user_stats):
        ...
        Where:
        - array_info is a list of ArrayInfo objects
